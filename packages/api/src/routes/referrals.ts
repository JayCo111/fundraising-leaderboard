// packages/api/src/routes/referrals.ts
import { Router, Request, Response } from 'express';
import { ReferralsService } from '../services/referralsService';
import { HTTP_STATUS, ERROR_MESSAGES } from '../utils/constants';

const router = Router();

/**
 * GET /api/v1/referrals
 * Get all referrals
 * Query params:
 * - stage: filter by stage
 * - studentId: filter by student ID
 * - days: get recent referrals (last N days)
 */
router.get('/', async (req: Request, res: Response) => {
  try {
    const { stage, studentId, days } = req.query;

    let referrals;

    if (days) {
      referrals = await ReferralsService.getRecentReferrals(Number(days));
    } else if (studentId) {
      referrals = await ReferralsService.getReferralsByStudentId(String(studentId));
    } else if (stage) {
      referrals = await ReferralsService.getReferralsByStage(String(stage));
    } else {
      referrals = await ReferralsService.getAllReferrals();
    }

    res.status(HTTP_STATUS.OK).json({
      success: true,
      data: referrals,
      count: referrals.length,
    });
  } catch (error: any) {
    console.error('Error in GET /referrals:', error);
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
      success: false,
      error: error.message || ERROR_MESSAGES.INTERNAL_SERVER_ERROR,
    });
  }
});

/**
 * POST /api/v1/referrals
 * Add new referral
 */
router.post('/', async (req: Request, res: Response) => {
  try {
    const result = await ReferralsService.addReferral(req.body);

    res.status(HTTP_STATUS.CREATED).json(result);
  } catch (error: any) {
    console.error('Error in POST /referrals:', error);
    res.status(HTTP_STATUS.BAD_REQUEST).json({
      success: false,
      error: error.message || ERROR_MESSAGES.INTERNAL_SERVER_ERROR,
    });
  }
});

/**
 * PUT /api/v1/referrals/:id
 * Update referral
 */
router.put('/:id', async (req: Request, res: Response) => {
  try {
    const result = await ReferralsService.updateReferral(req.params.id, req.body);

    res.status(HTTP_STATUS.OK).json(result);
  } catch (error: any) {
    console.error('Error in PUT /referrals/:id:', error);
    res.status(HTTP_STATUS.BAD_REQUEST).json({
      success: false,
      error: error.message || ERROR_MESSAGES.INTERNAL_SERVER_ERROR,
    });
  }
});

/**
 * GET /api/v1/referrals/student/:studentId
 * Get referrals for a specific student
 */
router.get('/student/:studentId', async (req: Request, res: Response) => {
  try {
    const referrals = await ReferralsService.getReferralsByStudentId(req.params.studentId);

    res.status(HTTP_STATUS.OK).json({
      success: true,
      data: referrals,
      count: referrals.length,
    });
  } catch (error: any) {
    console.error('Error in GET /referrals/student/:studentId:', error);
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
      success: false,
      error: error.message || ERROR_MESSAGES.INTERNAL_SERVER_ERROR,
    });
  }
});

/**
 * GET /api/v1/referrals/student/:studentId/stats
 * Get referral statistics for a student
 */
router.get('/student/:studentId/stats', async (req: Request, res: Response) => {
  try {
    const stats = await ReferralsService.getStudentReferralStats(req.params.studentId);

    res.status(HTTP_STATUS.OK).json({
      success: true,
      data: stats,
    });
  } catch (error: any) {
    console.error('Error in GET /referrals/student/:studentId/stats:', error);
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
      success: false,
      error: error.message || ERROR_MESSAGES.INTERNAL_SERVER_ERROR,
    });
  }
});

/**
 * GET /api/v1/referrals/leaderboard
 * Get referral leaderboard
 * Query params:
 * - limit: number of top students to return (default: 10)
 */
router.get('/leaderboard', async (req: Request, res: Response) => {
  try {
    const limit = req.query.limit ? Number(req.query.limit) : 10;
    const leaderboard = await ReferralsService.getReferralLeaderboard(limit);

    res.status(HTTP_STATUS.OK).json({
      success: true,
      data: leaderboard,
      count: leaderboard.length,
    });
  } catch (error: any) {
    console.error('Error in GET /referrals/leaderboard:', error);
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
      success: false,
      error: error.message || ERROR_MESSAGES.INTERNAL_SERVER_ERROR,
    });
  }
});

/**
 * GET /api/v1/referrals/stats/overall
 * Get overall referral statistics
 */
router.get('/stats/overall', async (req: Request, res: Response) => {
  try {
    const stats = await ReferralsService.getOverallStats();

    res.status(HTTP_STATUS.OK).json({
      success: true,
      data: stats,
    });
  } catch (error: any) {
    console.error('Error in GET /referrals/stats/overall:', error);
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
      success: false,
      error: error.message || ERROR_MESSAGES.INTERNAL_SERVER_ERROR,
    });
  }
});

export { router as referralsRoutes };
